...
main() {
    ...
    if [[ -n "$(bash -n "$settings_file" 2>&1)" ]]; then
        throw_error "The settings file $settings_file contains syntax errors. Fix the errors or remove the file." "$(bash -n "$settings_file" 2>&1)"
    fi
    ...
    if [[ -n "$(source "$settings_file" 2>&1)" ]]; then
        throw_error "The settings file $settings_file contains syntax errors. Fix the errors or remove the file." "$(source "$settings_file" 2>&1)"
    fi
    # shellcheck disable=SC1090
    ...
    *)
        ...
        *)
            case "$#" in
            1)
                throw_error_usage "$1 is not a valid argument" "Tried calling function with invalid argument $1"
                ;;
            *)
                throw_error_usage "$* are not a valid arguments" "Tried calling function with invalid arguments $*"
                ;;
            esac
            ;;
        esac
...
add() {
    ...
    if [ "$#" -lt 2 ]; then
        throw_error_usage "Missing description" "Tried adding task without description."
    fi
...
check_content() {
    while [ "$#" -gt 0 ]; do
        ...
        -i) # inline
            ...
            if [ "$status" -eq 1 ]; then
                throw_error_usage "Task must at least contain one word or a tag. Tasks that consist only out of contexts or dates have no meaning" "Tried adding meaningless tag $1"
            fi
            ;;
        -t) # task file
            touch "$tempfile"
...
check_date() {
    ...
    if [ -n "$datum" ]; then
        ...
        if ! date --date="$datum" &>/dev/null; then
            if [ "${!#}" -eq 0 ]; then
                throw_error_usage "Invalid date: $datum" "Tried adding task with invalid date: $datum"
            else
                echo False
            fi
        fi
...
delete_all_tasks() {
    rm "$TASK_FILE"
    # no need to create the file again, this is done by ensure_task_file
}
...
delete_task() {
    ...
    if [ "$#" -ne 2 ]; then
        throw_error_usage "Needs exactly one argument" "Tried passing "$#" arguments into delete_task()"
    fi
...
list_logs() {
    if [ "$#" -eq 1 ]; then
        ...
        throw_error_usage "list-logs recuires one integer argument" "Tried calling '$*', incorrect parameters."
    fi
...
remove_duplicate_delim() {
    if [ "$#" -eq 1 ]; then
        ...
        echo "${@:2}" | sed ":a; s/\($1\b\w\+\b\)\(.*\1\b\)/\2/g; ta; s/  / /g ;ta"
    fi
}
...
search() {
    if [ "$#" -ne 2 ]; then
        throw_error_usage "Search needs exactly one argument" "Tries calling Search with $# arguments"
    fi
    grep --color=always "$2" "$TASK_FILE"

}

# Usage: throw errors, and tell the user to look up usage of the script
throw_error_usage() {
